...
// 引入 hooks
import { useMangerGroupsBase } from "@/hooks/manager/recycle/groups/useMangerGroupsBase"
// 引入 接口api
...
const handlerReq = async () => {
  ...
  if (tableData.value.length === 1) {
    // 跳到上一页
    await reqGroups(pre, pageSize.value)
    return
  }
  // 处理批量删除时的逻辑
  ...
  if (len >= pageSize.value) {
    // 跳到上一页
    await reqGroups(cur - 1, pageSize.value)
    return
  }
  // 默认是 当前页 和分页器的个数
...
// 恢复
const handlerRestore = async (row: Group) => {
  const { id, name } = row
  try {
    await managerRestoreGroup(id)
    // 重新请求
    await handlerReq()
    ElMessage.success(`恢复权限组${name}成功~`)
  } catch (error) {
    ElMessage.error(`恢复权限组${name}失败~`)
  }
}

// 批量恢复
const handlerAllRemove = async () => {
  if (!userIds.value?.length) return ElMessage.warning("没有需要恢复的权限组")
  try {
    await Promise.all(
      userIds.value.map(async (item) => {
        try {
          await managerRestoreGroup(item)
        } catch (error) {
    ...
    // 重新请求
    if (pagination.value?.total === 1) await reqGroups()
