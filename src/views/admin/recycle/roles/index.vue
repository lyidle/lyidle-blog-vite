...
<script setup lang="ts" name="AdminRecycleRoles">
import moment from "@/utils/moment"
// 引入 hooks
import { useMangerRolesBase } from "@/hooks/manager/recycle/roles/useMangerRolesBase"
// 引入 接口api
import { managerRestoreRole } from "@/api/recycle"
// 引入 类型
import type { Role } from "@/api/recycle/types/getRecycleRole"
...
// 恢复
const handlerRestore = async (row: Role) => {
  const { id, account } = row
  try {
    await managerRestoreRole(id)
    // 重新请求
    await handlerReq()
    ElMessage.success(`恢复角色${account}成功~`)
  } catch (error) {
    ElMessage.warning(`恢复角色${account}失败~`)
  }
}

// 批量恢复
const handlerAllRemove = async () => {
  if (!userIds.value?.length) return ElMessage.warning("没有需要恢复的角色")
  try {
    await Promise.all(
      userIds.value.map(async (item) => {
        try {
          await managerRestoreRole(item)
        } catch (error) {
